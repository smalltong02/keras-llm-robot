...
- 🚀 2024-06-30更新日志
1. 支持新模型 **`gemma-2-9b-it`**, **`gemma-2-27b-it`**，需要升级**`transformers`** 库到 **`4.42.3`** (pip install transformers==4.42.3)
2. 支持新模型 **`Qwen2-0.5B-Instruct`**, **`Qwen2-1.5B-Instruct`**, **`Qwen2-7B-Instruct`**
...
2. 支持 **`together ai`** 云平台 和 **`firework ai`** 云平台。

- 🚀 2024-06-15更新日志
  1. 重构代码，主界面支持 **`"LLM 对话"`**，**`"知识库对话"`**，**`"文件对话"`** 和 **`"代理对话"`** 模式。
   
     在 **`"LLM 对话"`**中，模型将使用所有激活的工具，来进行辅助对话。

     在 **`"知识库对话"`**中，模型将仅使用选定的知识库来进行对话，所有激活的其它工具将处于禁用状态。

     在 **`"文件对话"`**中，模型将仅使用选定的文件来进行对话，所有激活的其它工具将处于禁用状态。

     在 **`"代理对话"`**中，模型将仅使用选定的代码解释器来完成任务，所有激活的其它工具将处于禁用状态。
     
  2. 支持 **`Gemini模型`**，**`GPT模型`** 和 **`Kimi模型`**的 **`原生Function Call`**功能。
  3. 添加 **`Keras代码解释器`**到工具箱中，当激活代码解释器的时候，在"LLM 对话"中，模型将自主决定是否要调用该工具。
